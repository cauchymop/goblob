...
abstract class GameRepository(
    ...
    protected abstract fun requestCacheRefresh(immediate: Boolean)

    protected fun saveToCache(gameData: GameData) {
        ...
        if (existingGame == null || gameData.sequenceNumber > existingGame.sequenceNumber) {
            gameCache.mutableGames[gameData.matchId] = gameData
            forceCacheRefresh()
    ...
    fun selectGame(matchId: String) {
        ...
        if (matchId == GameDatas.NEW_GAME_MATCH_ID) {
            ...
            fireGameSelected(gameCache.games[matchId])
        }
    }
